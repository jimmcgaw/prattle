...
(function(){
  ...
  class SearchInput extends React.Component {
    render(){
      return (
        <input type="search" id="search" placeholder="Say something" style={searchInputStyles} onChange={this.props.onChange} />
      );
    }
  }
  ...
  class TagRenderer extends React.Component {
    render(){
      return (
        <div id="tags" style={tagContainerStyles}>
            {this.props.tags.map(function(tag){
              return (
                <div className="tag" style={tagStyles}>
                  <h4>{ tag.text }</h4>
                  <ul>
                    <li>{ tag.pos.tag }</li>
                    <li>{ tag.pos.parent }</li>
                    <li>{ tag.pos.name }</li>
                  </ul>
                </div>
                );
  ...
  class POSTagger extends React.Component {
    ...
    render(){
      return (
        <div>
          <SearchInput onChange={this.onTextChange.bind(this)} />
          <TagRenderer tags={this.state.tags} />
        </div>
      );
    }
  }
